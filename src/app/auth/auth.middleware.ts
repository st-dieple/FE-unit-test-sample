...
import { signInError, signInSuccess, signOutError, signOutSuccess, signUpError, signUpSuccess } from './auth.actions';
...
export function* signOut() { 
  try {
    ...
    const res: AxiosResponse<any> = yield axios.post(
      `${environment.apiBaseUrl}${ENDPOINT.users.logout}`, '', { headers: { Authorization: `Bearer ${token}` } }
    );
